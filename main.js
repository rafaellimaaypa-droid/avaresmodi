...
function abrirModalUploadItemPremium(scene) {
    ...
    fileInput.onchange = (e) => {
        ...
        reader.onload = (event) => {
            selectedBase64 = event.target.result;
            const imgEl = document.getElementById('adminPremItemPreview');
            const phEl = document.getElementById('adminPremItemPlaceholder');
            if (imgEl) {
                imgEl.src = selectedBase64;
...
function update() {
    ...
    try {
        // Sincronização de acessórios (asas, etc)
        if (player.accessory && player.accessory.active) {
            ...
            player.accessory.setVisible(player.visible);
            player.accessory.setAlpha(player.alpha);
        }
        ...
        if (player.wingsLayerSprite && player.wingsLayerSprite.active) {
            try {
                if (!this.textures.exists(player.wingsLayerSprite.texture?.key)) {
                    player.wingsLayerSprite.destroy();
                    player.wingsLayerSprite = null;
                } else {
                ...
                if (player.wingsLayerSprite) {
                    try { player.wingsLayerSprite.destroy(); } catch (err) {}
                    player.wingsLayerSprite = null;
                }
            }
        }
        if (player.clothesLayerSprite && player.clothesLayerSprite.active) {
            try {
                if (!this.textures.exists(player.clothesLayerSprite.texture?.key)) {
                    player.clothesLayerSprite.destroy();
                    player.clothesLayerSprite = null;
                } else {
                ...
                if (player.clothesLayerSprite) {
                    try { player.clothesLayerSprite.destroy(); } catch (err) {}
                    player.clothesLayerSprite = null;
                }
            }
        }
    } catch (layerErr) {
        // Fallback silencioso: se houver qualquer erro em camadas, a renderização do corpo base continua normalmente
    }
